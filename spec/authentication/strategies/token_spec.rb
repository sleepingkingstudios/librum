...
RSpec.describe Authentication::Strategies::Token do
  ...
  describe '.token' do
    it { expect(described_class).to respond_to(:token).with(1).argument }

    it { expect(described_class.token(request)).to be nil }
    ...
    wrap_context 'with a request with a bearer Authorization header' do
      it { expect(described_class.token(request)).to be == token }
    end
